...
class TestHierarchicalRecursivelyComposes:
    def test_one_create_supervisor_call_per_team_plus_one_for_top(
        self, make_team: Callable[..., TeamSpec]
    ) -> None:
        teams = [make_team("billing"), make_team("tech_support")]
    ...
    def test_each_team_supervisor_uses_team_specialists(
        self, make_team: Callable[..., TeamSpec]
    ) -> None:
        teams = [make_team("billing")]
        with patch(
            "langgraph_forge.builders.multiagent.hierarchical.create_supervisor_agent",
            return_value=sentinel.compiled,
        ) as mock_create:
    ...
    def test_each_team_supervisor_uses_team_model_and_prompt(
        self, make_team: Callable[..., TeamSpec]
    ) -> None:
        team = make_team("billing")
        with (
            patch(
                "langgraph_forge.builders.multiagent.hierarchical.create_supervisor_agent",
                return_value=sentinel.compiled,
...
class TestTopSupervisorReceivesTeamSubgraphs:
    ...
    def test_top_supervisor_receives_top_model_and_prompt(
        self, make_team: Callable[..., TeamSpec]
    ) -> None:
        with patch(
            "langgraph_forge.builders.multiagent.hierarchical.create_supervisor_agent",
...
class TestTopLevelCrossCuttingForwarding:
    def test_top_level_checkpointer_lands_on_top_spec(
        self, make_team: Callable[..., TeamSpec]
    ) -> None:
    ...
    def test_top_level_interrupts_land_on_top_spec(
        self, make_team: Callable[..., TeamSpec]
    ) -> None:
        with patch(
            "langgraph_forge.builders.multiagent.hierarchical.create_supervisor_agent",
    ...
    def test_returns_top_supervisor_compiled_graph(
        self, make_team: Callable[..., TeamSpec]
    ) -> None:
        with patch(
            "langgraph_forge.builders.multiagent.hierarchical.create_supervisor_agent",
